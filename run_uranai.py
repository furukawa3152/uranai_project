...
if today != last_fetch:
    ...
    print("見えました！！")
    with open("last.fetch.csv", mode="w", encoding="ANSI", newline="")as f:
        f.write(today)


def run_uranai(sign: int, bloom: int):
    fortune_list = []
    sign_dict = {1: "牡羊座", 2: "牡牛座", 3: "双子座", 4: "蟹座", 5: "獅子座", 6: "乙女座", 7: "天秤座", 8: "蠍座", 9: "射手座", 10: "山羊座",
                 11: "水瓶座", 12: "魚座"}
    bloom_dict = {1: "A型", 2: "B型", 3: "O型", 4: "AB型"}
    sign = sign_dict[sign]
    bloom = bloom_dict[bloom]
    with open("fortune.csv", "r") as f:
        ...
        for i in range(len(fortune_list)):
            if sign == fortune_list[i][5] and bloom == fortune_list[i][6]:
